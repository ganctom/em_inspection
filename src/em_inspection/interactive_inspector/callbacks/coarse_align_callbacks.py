...
@callback(
    [
        Output({'type': UI.TYPE_ACQ_CFG_FIELD, 'index': ALL}, "value", allow_duplicate=True),
        Output({'type': UI.TYPE_REG_CFG_FIELD, 'index': ALL}, "value", allow_duplicate=True),
        Output({'type': UI.TYPE_MESH_CFG_FIELD, 'index': ALL}, "value", allow_duplicate=True),
        Output({'type': UI.TYPE_WARP_CFG_FIELD, 'index': ALL}, "value", allow_duplicate=True),
        Output({'type': UI.TYPE_MASK_CFG_FIELD, 'index': ALL}, "value", allow_duplicate=True),
...
def handle_config_load(n_clicks, file_path, *layout_ids_lists):
    ...
    try:
        ...
        return [*layout_payloads, "Loaded successfully", f"Active: {filename}", success_log]
...
def run_coarse_alignment(n_clicks, range_str, config_path, *args, stitch_config=None):
    ...
    if isinstance(stitch_config, ValidationError):
        err_details = "; ".join([f"['{'->'.join(map(str, e['loc']))}']: {e['msg']}" for e in stitch_config.errors()])
        log_err = UI.log_row(f"Initialization Failed: Schema validation failed -> {err_details}", type="error")
...
def unified_progress_poller(n, current_log_components):
    raw_text_history = []
    current_components = current_log_components if isinstance(current_log_components, list) else []
    ...
    prog, msg, disable_poller, updated_text_history = CoarseAlignManager.poll_unified_progress(
        raw_text_history
    )
