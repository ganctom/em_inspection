...
def create_grid_navigator(
    ...
    if tile_ids.ndim == 1:
        logging.warning(
            "Grid Navigator received a 1D array. Check initialization lifecycle pipes."
        )
        return go.Figure()
    ...
    fig = go.Figure(
        data=go.Heatmap(
            z=z,
            hoverinfo="skip",
            colorscale=[[0, UI.CLR_GRID_BASE_HTMP], [1, UI.CLR_GRID_BASE_HTMP]],
            showscale=False,
            xgap=2,
            ygap=2,
        )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            textfont=dict(
                family="Arial", size=UI.SIZE_TEXT_GRID_TILE_ID, color=text_colors
            ),
            marker=dict(
                symbol="square",
                ...
                [
                    ...
                    html.P(
                        [
                            ...
                            html.Span(
                                exp_config.proc_dir, className="text-break small"
                            ),
                        ],
                        className="mb-2",
                    ),
