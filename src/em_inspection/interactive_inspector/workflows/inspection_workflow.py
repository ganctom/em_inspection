...
from interactive_inspector.constants import (
    UIConstants,
    OverlapType,
    UI,
    KeyboardShortcuts,
...
class InspectionWorkflowManager:
    ...
    def safe_extract_pattern_index(
        triggered_id: any, selection_data: list
    ) -> dict | None:
        """
        ...
        clicked_idx = (
            triggered_id.get("index") if isinstance(triggered_id, dict) else None
        )
        if clicked_idx is None or clicked_idx >= len(selection_data):
            logging.warning(
                f"Workflow: Extraction aborted. Index pointer out of bounds: {clicked_idx}"
            )
            return None
    ...
    def handle_flow_visualization(
        ...
        trig_type = (
            triggered_id.get("type") if isinstance(triggered_id, dict) else triggered_id
        )
        ...
        if trig_type == UIConstants.ID_BTN_CLEAN_FLOW:
            ui_config = RegistrationConfig(
                **settings_data["registration_config"]
            )  # TODO
            do_clean_flow = True
    ...
    def handle_range_masks_visualization(
        cls, triggered_id: any, triggered_events: list, selection_data: list
    ):
        """
        ...
        fig = service.get_range_masks_fig(
            section_num=int(item_z), tile_id_num=int(item_tid)
        )
        if fig is None:
            error_msg = (
                service.message_queue.pop()
            ...
            return (
                no_update,
                "Dynamic Range Masks Visualization Error",
    ...
    def handle_raw_tile_visualization(
        ...
        try:
            ...
            if fig is None:
                error_msg = (
                    service.message_queue.pop()
                    if getattr(service, "message_queue", None)
                    else "Unknown Error"
                )
                return (
                    no_update,
                    "Tile Image Load Error",
                    html.Div(error_msg, className="text-danger"),
                )
            ...
            return (
                no_update,
                f"Visualizer Crash: {str(e)}",
                html.Div(str(e), className="text-danger"),
            )

    @classmethod
    def handle_tile_overlap_replot(
        cls, active_idx: int, nudge_trigger: any, selection_data: list
    ):
        ...
        try:
            ...
            return no_update, html.Div(
                f"Rendering Error: {str(e)}", className="text-danger"
            )

    @classmethod
    def handle_single_calculation(
        cls,
        triggered_id: any,
        triggered_events: list,
        selection_data: list,
        ...
        safe_nudge = (
            nudge_trigger if isinstance(nudge_trigger, dict) else {"dx": 0, "dy": 0}
        )
        ...
        if isinstance(result, str):
            return (
                html.Div(result, className="text-danger"),
                no_update,
                "Refinement Failed",
            )
        ...
        return (
            html.P(
                f"T{calc_item['tid']} Refined: {result['refined']}",
    ...
    def handle_selection_state_mutation(
        ...
        if isinstance(triggered_id, dict) and triggered_id.get("type") == "remove-btn":
            return [
                item
                for i, item in enumerate(current_store)
                if i != triggered_id.get("index")
            ]
    ...
    def sync_basket_ui_container(data: list):
        ...
        if not data:
            return html.Div(
                "No vectors selected.", className="text-muted small italic p-2"
            )
        return [selection_card(i, item) for i, item in enumerate(data)]

    @classmethod
    def handle_main_quad_visualization(
        cls, grid_click: dict, selection_store: list, dark_mode: list
    ):
        ...
        current_tid_selections = [
            s for s in selection_store if str(s["tid"]) == str(raw_tid)
        ]
        for pt in current_tid_selections:
            try:
                ...
                is_h = pt_ov == OverlapType.HORIZONTAL or str(pt_ov).endswith(
                    "HORIZONTAL"
                )
                ...
                marker_style = dict(
                    size=14, color=h_color, symbol="circle-open", line=dict(width=2)
                )
    ...
    def handle_batch_calculation(
        ...
        log_entries = [
            html.Div(
                f"T{s[0]['tid']}: {s[1]['refined']}"
                if isinstance(s[1], dict)
                else f"T{s[0]['tid']}: FAILED",
                className="text-success small"
                if isinstance(s[1], dict)
                else "text-danger small",
            )
            for s in results
        ]
    ...
    def handle_grid_navigation(
        ...
        if not z_keys:
            logging.warning(
                "Grid Navigator Workflow: Action aborted. Section sequence empty."
            )
            return no_update, no_update, no_update
        ...
        if triggered_id == "manual-z-input" and manual_z is not None:
            ...
            current_z = (
                (z_max + z_min) - slider_val if slider_val is not None else z_min
            )
        ...
        try:
            ...
            logging.warning(
                f"Workflow Map Refinement Failure on slice {current_z}: {e}"
            )
            available_tids = set()
    ...
    @classmethod
    def handle_keyboard_slice_navigation(
        cls, n_events: int, event: dict, current_slider_val: int
    ):
    ...
    @classmethod
    def handle_nudge_and_basket_navigation(
        cls,
        triggered_id: any,
        key_event: dict,
        step_list: list,
        ...
        if (
            triggered_id == "keyboard-listener"
            and key_event
...
def apply_padded_y_ranges(
    fig: go.Figure, shifts: npt.NDArray[np.float64], padding_factor: float
) -> None:
    """
    ...
    for (r, c), matrix_idx in subplot_matrix_mapping.items():
        ...
        if grid_ref is not None:
            try:
                ...
                logging.debug(
                    f"Grid reference extraction failed, falling back to update. Error: {e}"
                )
