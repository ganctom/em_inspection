...
def eval_dyn_range(a: np.ndarray, range_limit=10, filter_size=10):
    dyn_range = ndimage.maximum_filter(a, filter_size) - ndimage.minimum_filter(
        a, filter_size
    )
    return dyn_range
...
def detect_smearing2d(
    img: np.ndarray, segment_width: int = 1000, dx: int = 10, dy: int = 3, sigma=1.5
) -> Optional[np.ndarray]:
...
def get_smearing_mask(
    img: np.ndarray, mask_top_edge: int = 0, path_plot: Optional[str] = None, plot=False
) -> Optional[np.ndarray]:
    ...
    det_args = dict(img=img, segment_width=1000, sigma=1.5, dx=50, dy=4)
    ...
    clean_args = dict(
        mask=mask, min_size=800, portion=1.0, max_vert_extent=1500, top=mask_top_edge
    )
