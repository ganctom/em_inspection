...
class LayoutFactory:
    ...
    def create_grid_navigator(tile_ids: np.ndarray) -> go.Figure:
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                autorange="reversed", scaleanchor="x", visible=False, fixedrange=True
            ),
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    [
                        html.H6(
                            "Navigation", className="mt-3 text-uppercase text-muted"
                        ),
                        dcc.Graph(
                            id="master-grid",
                            figure=LayoutFactory.create_grid_navigator(
                                data_service.tile_ids
                            ),
...
def handle_selection_state(sel_data, clear_n, remove_n, current_store, grid_click):
    ...
    if isinstance(trigger, dict) and trigger.get("type") == "remove-btn":
        return [
            item for i, item in enumerate(current_store) if i != trigger.get("index")
        ]
