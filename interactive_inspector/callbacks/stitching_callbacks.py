...
def start_stitching_pipeline(n_clicks, use_parallel, range_str, selected_steps, config_path):
    ...
    thread = threading.Thread(
        target=target_method, # <--- The New Method
...
# def start_stitching_pipeline(n_clicks, par_sec, range_str, selected_steps, config_path):
#     if not selected_steps:
#         return [UI.log_row("Error: No steps selected.", type="error")], 0, True
#
#     if not dat_service.exp_config:
...
#     try:
#         first, last = dat_service.exp_config.first_sec, dat_service.exp_config.last_sec
...
#     try:
#         final_config = dat_service.prepare_stitching_params(config_path, ui_params=None)
#     except Exception as e:
